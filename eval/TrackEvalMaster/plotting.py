...
def create_comparison_plot(data, out_loc, y_label, x_label, sort_label, bg_label=None, bg_function=None, settings=None):
    ...
    id_to_name={}
    name_to_sn={'city_vps':'city_vps','kitti_step':'kitti_step','waymo':'waymo','VPSNet':'vpsnet','pcan':'pcan','botsort_deepmac':'botsort'}
    for i, name in enumerate(tracker_names):
        print('%i: %s' % (i+1, name))        
        id_to_name.update({i+1 : name_to_sn[name]})
    ...
    #edit by subha starts
    sup_title=name_to_sn[out_loc.split('/')[-2]]
    #sup_title = '_'.join((out_loc.split('/')[-2].split('_')[:-1]))
    plt.text(0.5,1.15 , sup_title, horizontalalignment='center', verticalalignment='center',
             transform=fig.axes[0].transAxes, color="green", fontsize=19)
    ...
    for xx, yy, l in zip(x_values, y_values, labels):
        ...
        plt.text(xx, yy, str(l)+'.'+id_to_name[l], color="red", fontsize=15)#--> edited by subha
        #print("****"+id_to_name[l])

    # Add extra explanatory text to plots
    la=os.path.basename(out_loc)
    #if la!='all':     
    plt.text(0.85, -0.15, 'curve values:\n' + bg_label, horizontalalignment='left', verticalalignment='center',
             transform=fig.axes[0].transAxes, color= "grey",fontsize=12)
    if bg_label is not None:
        plt.text(0.25, -0.15, 'label order :\nSTQ', horizontalalignment='right', verticalalignment='center',
                 transform=fig.axes[0].transAxes, color= "red",fontsize=12)

    plt.xlabel(x_label, fontsize=15)
    plt.ylabel(y_label, fontsize=15)
    
    #plt.suptitle(sup_title,color="green", fontsize=19)
