...
if __name__ == '__main__':
    ...
    script_config = {
        ...
        'BENCHMARKS': ['city_vps_mots','kitti_step_mots','waymo_mots'],  # If None, use all for each split.
        ...
        'TRACKERS_TO_EVAL':['pcan', 'VPSNet', 'botsort_deepmac'],
    }
    ...
    default_dataset_config = trackeval.datasets.CityKitti.get_default_dataset_config()
    ...
    if not config['BENCHMARKS']:
        if config['CK_SPLIT'] == 'val':
            config['BENCHMARKS'] = ['city_vps_mots','kitti_step_mots','waymo_mots']
    ...
    try:
        ...
        for bench in config['BENCHMARKS']:
            ...
            dataset_list.append(trackeval.datasets.CityKitti(dataset_config))
    ...
    if success:
        ...
        #print(output_res)
        trackers = list(output_res['CityKitti.' + config['BENCHMARKS'][0]].keys())
        for tracker in trackers:
            # final_results[benchmark][result_type][metric]
            final_results = {}
            res = {bench: output_res['CityKitti.' + bench][tracker]['COMBINED_SEQ'] for bench in config['BENCHMARKS']}
            with open(os.path.join(config['TRACKERS_FOLDER'], config['CK_SPLIT'], tracker,'combined_results.txt'),"w") as f:
                pprint.pprint(res,f)
            with open(os.path.join(config['TRACKERS_FOLDER'], config['CK_SPLIT'], tracker,'combined_results.pkl'),"wb") as f:
                pickle.dump(res,f)
