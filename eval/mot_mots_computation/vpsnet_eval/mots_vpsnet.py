...
from mot_vpsnet import (xyxy2xywh, preprocessResult, aggregate_eval_results)
...
def eval_mots(anns, all_results, split_camera=False, class_average=False):
    ...
    for img, results in zip(anns['images'], all_results):
        ...
        if len(list(results.keys())) > 0:#result.keys--> num of instances in an img --> subha
            num_instances = max([int(k) for k in results.keys()]) + 1 #since objs starts with 0

        pred_bboxes, pred_ids = defaultdict(list), defaultdict(list)
        pred_segms = defaultdict(list)
        for instance_id, result in results.items():#instance_id is what i consider object id -->subha
            
            #_bbox = xyxy2xywh(result['bbox']) --> original
            _bbox = result['bbox'] #vpsnet has xywh in pkl file
            '''
            _cat = cats_mapping[result['label']+1]#pcan network gives cat id from 0 as pedestrian(found at quasi_dense_pcan_seg--> write_result function ) . gt ann has cat ids starting from 1 as pedestrian. so increamenting i assume-->subha	
            '''
            _cat=cats_mapping[result['label']] #--> for vpsnet mask


            pred_bboxes[_cat].append(_bbox)# e.g collecting all cars in an image under pred_bboxes['car']
            instance_id = int(instance_id) + global_instance_id
            pred_ids[_cat].append(instance_id)
            #pred_segms[_cat].append(result['segm'])   ---> original
            pred_segms[_cat].append(annToRLE(result['segm']))#-->edited by subha for vpsnet masks
        ...
        for cat_id in cat_ids:
            for i in annsByAttr[img_id][cat_id]:
                ...
                gt_segms[cat_id].append(annToRLE(ann['segmentation']))
