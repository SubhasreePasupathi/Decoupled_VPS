...
path='waymo_modified_with_cv_ids'
l=os.listdir(path)
l.sort()
...
for i,fol in enumerate(l):
    sid=fol
    print(i)
    ll=os.listdir(os.path.join(path,fol,'pan_2ch'))
    ll.sort()
    for j,img_name in enumerate(ll):
        img=Image.open(os.path.join(path,fol,'pan_2ch',img_name))
        arr=np.array(img)
        ...
        for ins_id in ins_ids:
            ...
            write_line(sid,j,ins_id,class_id,str('1'),img.size[1],img.size[0],rle.decode('UTF-8'))
