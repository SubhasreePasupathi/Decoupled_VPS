...
def create_json(im_path_list,sn):
    pred_pan_2ch=[]
    for i,image in enumerate(im_path_list):
    	im=Image.open(os.path.join(path,sn,image))
    ...
    for idx in range(len(pred_pan_2ch)):
        ...
        for el in l:
            ...
            if el % OFFSET > 0: #-->original commented by subha kitti step obj ids start from 0
            #if sem>10:
            ...
            dt = {"category_id": sem.item(), "iscrowd": 0, "id": int(rgb2id(color)), "color":[int(x) for x in color], "bbox": [x.item(), y.item(), width.item(), height.item()], "area": mask.sum().item()}
        ...
        annotations.append({"image_id":'_'.join(im_path_list[idx].split('_')[:-1]),"file_name":im_path_list[idx] ,"segments_info": segm_info})
        images.append({"id": '_'.join(im_path_list[idx].split('_')[:-1]), "width":w, "height":h,"file_name": im_path_list[idx]})
    json_dict={'annotations':annotations,'images':images,'categories':categories}
    ...
    if not os.path.isdir('gt/gt_ann_jsons_bb'):
        os.makedirs('gt/gt_ann_jsons_bb')
    with open(os.path.join('gt/gt_ann_jsons_bb/pan_gt_waymo_annotaions_'+sn+'.json'),'w') as ff:
    	json.dump(json_dict,ff)
    
    save_path=os.path.join('gt/colour_map_waymo_gt_for_vpq',sn)
...
fol_path_list=os.listdir(path)
fol_path_list.sort()

for fol in fol_path_list:
    im_path_list=os.listdir(os.path.join(path,fol))
    im_path_list.sort()
    print(fol)
    create_json(im_path_list,fol)
